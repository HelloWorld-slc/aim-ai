...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(list.tools.map(t => t.name).sort(), ['robot_batch', 'robot_connect', 'robot_disconnect', 'robot_snapshot', 'robot_stop', 'robot_test']); passed.push('six gateway tools; no shell, files, continuous movement or raw upstream tools');
    ...
    const batchDenied = await client.client.callTool({ name: 'robot_batch', arguments: { steps: [{ kind: 'move', amount: 50, speed: 20 }], approval: 'wrong' } });
    assert.equal(batchDenied.isError, true); assert.equal((await client.snapshot()).positionMm.y, 0); passed.push('batch requires host-held approval token');
    const narrow = await client.snapshot(false, undefined, ['batteryPercent']);
    assert.equal(narrow.batteryPercent, 80); assert.equal(narrow.positionMm, undefined); assert.equal(narrow.mode, 'simulation');
    assert.ok((await client.snapshot(false, undefined, 'all')).vision); passed.push('full and partial fields roundtrip through MCP');
    const batch = await client.batch({ steps: [{ kind: 'move', amount: 30, speed: 20 }, { kind: 'move', amount: 30, speed: 20, direction: 'backward' }], fields: ['positionMm'] });
    assert.equal(batch.status, 'completed'); assert.equal(batch.completedSteps, 2); assert.equal(batch.net.displacementMm, 0); assert.equal(batch.after.batteryPercent, undefined); passed.push('one batch returns projected state and telemetry comparisons');
