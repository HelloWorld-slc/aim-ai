...
(async () => {
  ...
  try {
    ...
    await page.locator('.robot-chat-options summary').click();
    assert.equal(await page.locator('#robot-autonomous').isChecked(), false);
    await page.locator('#robot-autonomous').check();
    await page.locator('#prompt').fill('测试一个短距离往返');
    await page.locator('#send').click();
    assert.ok(await page.evaluate(() => window.sent.some(m => m.type === 'send' && m.robotAutonomous === true && m.robotDisabled === false)));
    assert.equal(await page.locator('#robot-autonomous').isChecked(), false);
    await page.locator('#robot-disabled').check();
    assert.equal(await page.locator('#robot-autonomous').isDisabled(), true);
    await page.locator('#prompt').fill('只编程'); await page.locator('#send').click();
    assert.ok(await page.evaluate(() => window.sent.some(m => m.type === 'send' && m.robotDisabled === true && m.robotAutonomous === false)));
    await page.setViewportSize({ width: 380, height: 1200 });
    await page.screenshot({ path: path.join(root, 'test-results', 'autonomous.png'), fullPage: true });
    await page.locator('#robot-disabled').uncheck();
    await page.locator('nav [data-tab="robot"]').click();
