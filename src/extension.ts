        ...
        void guard(async () => {
          switch (message.type) {
            case 'ready':
              ...
              break;
            case 'send': if (typeof message.prompt === 'string') await assistant.send(message.prompt, message.robotDebug === true, message.robotAutonomous === true, message.robotDisabled === true); break;
...
function html(nonce: string, csp: string, script: string, style: string): string {
  ...
  <main>
    <section id="chat" class="page active"><div class="model-row"><button id="model" class="model-select" data-tab="models"><img id="model-icon" alt="" hidden><span id="model-name">尚未配置</span><span aria-hidden="true">⌄</span></button><button class="text-button" data-command="testConnection">测试连接</button></div><div class="thinking-row"><label for="thinking-effort">思考强度</label><select id="thinking-effort"><option value="default">预设默认</option></select></div><p id="thinking-note" class="muted thinking-note"></p><div class="chat-heading"><span id="chat-title">新对话</span><span class="muted">本地保存</span></div>
      ...
      <details class="robot-chat-options"><summary>机器人辅助与自主调试</summary><label class="robot-assist-option"><input id="robot-assist" type="checkbox" disabled> 本轮读取状态、提出测试建议</label><label class="robot-assist-option"><input id="robot-autonomous" type="checkbox" disabled> 本轮允许 AI 自主运动调试</label><p class="muted">勾选后本轮可直接执行一次多步测试，不再逐步确认。先放好机器人；发送后选项自动取消勾选。最多 8 步，可随时取消。</p><label class="robot-assist-option"><input id="robot-disabled" type="checkbox"> 本轮不使用 MCP（只编程）</label></details><div class="composer"><label class="sr-only" for="prompt">编程任务</label><textarea id="prompt" rows="4" maxlength="8000" placeholder="例如：找到足球后转向它；球丢失时停止前进…"></textarea><div class="composer-footer"><button class="text-button" data-action="clear" title="开始新对话，已有记录保留在历史中">＋ 新对话</button><span id="request-status"></span><button id="cancel" data-action="cancel" hidden>取消</button><button id="send" class="primary" data-action="send">发送 ↑</button></div></div><p class="footnote">对话自动保存在本机。发送时会将当前程序、近期对话与相关资料交给所选模型服务。</p>
    </section>
    ...
    <section id="docs" class="page"><div class="section-intro"><span class="eyebrow">随插件提供 · 离线可读</span><h2>AIM 参考资料</h2><p>包含 Python 全部 15 个分类、Logic 的 12 个子页及图像识别专题。每项提供中文资料与来源链接。</p></div><label class="sr-only" for="topic-search">搜索资料、方法名或关键词</label><input id="topic-search" type="search" placeholder="搜索：视觉、线程、draw_rectangle…" autocomplete="off"><p id="topic-count" class="muted" role="status" aria-live="polite"></p><div id="topics"></div><div class="card quiet"><p>AI 按任务选取相关资料，也能继续查阅其他条目。MicroPython 库页附 14 个模块的官方文档链接。</p><p>最近更新：2026-09-21。各页保留核对日期，完整参数、示例与任务卡可从官方链接查看。</p></div></section>
